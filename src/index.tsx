    ...
    if (window && window.matchMedia) {
      ...
      breakPoints.forEach((bp, index) => {
        mediaQueryLists.push(
          window.matchMedia(`screen and (min-width: ${bp}px)${breakPoints[index + 1] ? ` and (max-width: ${breakPoints[index + 1] - 1}px` : ''}`)
        );
...
const useStyledSystem = (
  props: unknown,
  { remBase = 10, fontSizes = [12, 14, 16, 20, 24, 32, 48, 64, 72], spaceSizes = [0, 4, 8, 16, 32, 64, 128, 256, 512], ...config }: ConfigProps
): UseStyledSystemProps => {
  ...
  }, [config, cssProps, fontSizes, remBase, spaceSizes]);
